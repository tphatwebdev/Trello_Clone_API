...
const createNewBoardInvitation = async (data) => {
  try {
    ...
    let newInvitationToAdd = {
      ...validData,
      inviterId: toObjectId(validData.inviterId),
      inviteeId: toObjectId(validData.inviteeId)
    }
    // nếu tồn tại dữ liệu boardInvitation thì update cho cái boardId
    if (validData.boardInvitation) {
      newInvitationToAdd.boardInvitation = {
        ...validData.boardInvitation,
        boardId: toObjectId(validData.boardInvitation.boardId)
      }
    }
    // Gọi insert vào DB
...
const findOneById = async (invitationId) => {
  try {
    const result = await GET_DB().collection(INVITATION_COLLECTION_NAME).findOne({ _id: toObjectId(invitationId) })
    return result
  } catch (error) {
    throw new Error(error)
  }
...
const update = async (invitationId, updateData) => {
  try {
    // lọc field khong cho phép cập nhật
    ...
    if (updateData.boardInvitation) {
      updateData.boardInvitation = {
        ...updateData.boardInvitation,
        boardId: toObjectId(updateData.boardInvitation.boardId)
      }
    }
...
const findByUser = async (userId) => {
  try {
    const queryConditions = [
      { inviteeId: toObjectId(userId) }, // tìm theo inviteeId - người được mời -> chính là người đang thực hiện request này
      { _destroy: false }
    ]
